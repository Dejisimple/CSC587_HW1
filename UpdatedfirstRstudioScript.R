...
su<-read.delim("C:\\Users\\HOME\\Downloads\\Su_raw_matrix.txt")
...
```{r quantile stuff}
su = read.delim("C:/Users/HOME/Downloads/Su_raw_matrix.txt", sep = ',', header=T)
...
```{r boxplot}
#boxplot
su <- read.delim("C:\\Users\\HOME\\Downloads\\Su_raw_matrix.txt.txt")
boxplot(su_raw_matrix.txt$plas)
hist(Su_raw_matrix.txt$plas)
head(Su_raw_matrix.txt)
```
